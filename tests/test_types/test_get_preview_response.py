...
from walletpay.types import GetPreviewResponse, GetPreviewStatus, OrderStatus, CurrencyCode
...
def test_method_compilation():
    ...
    response = GetPreviewResponse.from_json(create_response_json)

    assert response.status == GetPreviewStatus.SUCCESS
